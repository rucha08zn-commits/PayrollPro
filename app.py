...
with app.app_context():
    db.create_all()
    # Seed demo data on a fresh deployment OR when an existing database has
    # employees but no attendance records. The latter can happen when an older
    # database is reused on Render, so employee self-service would otherwise
    # show blank attendance/present/absent data.
    from seed_data import seed_demo_data, ensure_current_month_demo_attendance
    if Employee.query.count() == 0 or Attendance.query.count() == 0:
        seed_demo_data()
    # Keep the employee dashboard useful in the current month as well. Only
    # populate demo rows when the current month has no attendance yet.
    ensure_current_month_demo_attendance()
...
def employee_dashboard():
    ...
    month_attendance = Attendance.query.filter(
        ...
        Attendance.date.like(f"{current_month}%")
    ).order_by(Attendance.date.desc()).all()
    punch_rows = PunchLog.query.filter(
        PunchLog.emp_id == emp.id,
        PunchLog.date.like(f"{current_month}%")
    ).all()
    punches = {p.date: p for p in punch_rows}
    attendance_rows = [(a, date.fromisoformat(a.date).strftime('%a'), punches.get(a.date)) for a in month_attendance]

    return render_emp(r"""
    <div class="pp-page-heading">
      <div><div class="pp-eyebrow">Employee Self Service</div>
      <h1 class="pp-title">Welcome, {{e.name.split()[0]}} 👋</h1>
      <div class="pp-subtitle">Your profile, salary and attendance — all in one place.</div></div>
      <a href="/employee/attendance" class="btn pp-btn-primary"><i class="ti ti-calendar-check me-1"></i> View Full Attendance</a>
    </div>

    <div class="row g-3 mb-3">
      <div class="col-lg-8"><div class="pp-card pp-card-pad h-100">
        <div class="d-flex align-items-center gap-3">
          <div class="pp-avatar" style="width:54px;height:54px;font-size:18px;">{{e.name[:1]}}</div>
          <div><h4 class="mb-1">{{e.name}}</h4><div class="text-muted small">EMP{{'%03d'|format(e.id)}} · {{e.designation}} · {{e.department}}</div>
          <div class="text-muted small mt-1">{{e.email}} · {{e.phone}}</div></div>
        </div>
      </div></div>
      <div class="col-lg-4"><div class="pp-card pp-card-pad h-100 text-center">
        <div class="pp-section-muted">My Net Salary</div><div class="fs-2 fw-bold">₹{{ '%.0f'|format(e.net) }}</div>
        <a href="/employee/payslip_page" class="btn btn-sm pp-btn-soft mt-1">View Payslip</a>
      </div></div>
    </div>

    <div class="row g-3 mb-3">
      <div class="col-6 col-lg-3"><div class="pp-card pp-stat" style="--stat-bg:#dcfce7;--stat-color:#15803d"><div class="pp-stat-icon"><i class="ti ti-circle-check"></i></div><div class="pp-stat-label">Present · {{current_month}}</div><div class="pp-stat-value">{{s.present_days}}</div></div></div>
      <div class="col-6 col-lg-3"><div class="pp-card pp-stat" style="--stat-bg:#fee2e2;--stat-color:#b91c1c"><div class="pp-stat-icon"><i class="ti ti-circle-x"></i></div><div class="pp-stat-label">Absent · {{current_month}}</div><div class="pp-stat-value">{{s.absent_days}}</div></div></div>
      <div class="col-6 col-lg-3"><div class="pp-card pp-stat" style="--stat-bg:#fef3c7;--stat-color:#b45309"><div class="pp-stat-icon"><i class="ti ti-clock-hour-4"></i></div><div class="pp-stat-label">Half Day · {{current_month}}</div><div class="pp-stat-value">{{s.halfday_days}}</div></div></div>
      <div class="col-6 col-lg-3"><div class="pp-card pp-stat" style="--stat-bg:#dbeafe;--stat-color:#1d4ed8"><div class="pp-stat-icon"><i class="ti ti-clock"></i></div><div class="pp-stat-label">Overtime · {{current_month}}</div><div class="pp-stat-value">{{s.overtime_hours}}h</div></div></div>
    </div>

    <div class="row g-3">
      <div class="col-xl-8"><div class="pp-card">
        <div class="pp-card-pad d-flex justify-content-between align-items-center">
          <div><h3 class="pp-section-title">📅 My Attendance — {{current_month}}</h3><div class="pp-section-muted">Date-wise attendance visible from your employee account.</div></div>
          <span class="pp-badge pp-badge-purple">{{month_attendance|length}} records</span>
        </div>
        <div class="table-responsive"><table class="table pp-table mb-0"><thead><tr><th>Date</th><th>Day</th><th>Status</th><th>In</th><th>Out</th><th>Hours</th><th>OT</th></tr></thead><tbody>
        {% for a,day_name,p in attendance_rows %}
          <tr><td><b>{{a.date}}</b></td><td>{{day_name}}</td>
          <td><span class="pp-badge {{'pp-badge-success' if a.status=='Present' else ('pp-badge-danger' if a.status=='Absent' else ('pp-badge-warning' if a.status=='Half Day' else 'pp-badge-gray'))}}">{{a.status}}</span></td>
          <td>{{p.time_in if p else '—'}}</td><td>{{p.time_out if p else '—'}}</td><td>{{p.hours if p and p.hours else '—'}}</td><td>{{p.overtime_hours if p and p.overtime_hours else '—'}}</td></tr>
        {% else %}<tr><td colspan="7" class="pp-empty">No attendance records found for {{current_month}}.</td></tr>{% endfor %}
        </tbody></table></div>
      </div></div>

      <div class="col-xl-4"><div class="pp-card pp-card-pad h-100">
        <h3 class="pp-section-title mb-3">Quick Details</h3>
        <div class="pp-list-item"><div class="pp-list-icon" style="background:#ede9fe;color:#7c3aed"><i class="ti ti-building"></i></div><div><b>Department</b><div class="pp-section-muted">{{e.department}}</div></div></div>
        <div class="pp-list-item"><div class="pp-list-icon" style="background:#dbeafe;color:#2563eb"><i class="ti ti-map-pin"></i></div><div><b>Work Location</b><div class="pp-section-muted">{{e.work_location or '—'}}</div></div></div>
        <div class="pp-list-item"><div class="pp-list-icon" style="background:#dcfce7;color:#15803d"><i class="ti ti-calendar"></i></div><div><b>Leave Balance</b><div class="pp-section-muted">{{leave_balance}} day(s) remaining</div></div></div>
        <div class="pp-list-item"><div class="pp-list-icon" style="background:#fef3c7;color:#b45309"><i class="ti ti-clock"></i></div><div><b>Today's Punch</b><div class="pp-section-muted">{{'Punched in at ' ~ open_log.time_in if open_log else 'Not punched in today'}}</div></div></div>
        <a href="/employee/profile" class="btn btn-outline-secondary w-100 mt-3">View My Full Profile</a>
      </div></div>
    </div>

    <div class="row g-3 mt-1">
      <div class="col-md-4"><div class="pp-card pp-card-pad h-100"><h3 class="pp-section-title">⏱ Punch & Attendance</h3><p class="text-muted small">Punch in/out and see complete attendance history.</p><a href="/employee/attendance" class="btn btn-sm pp-btn-primary w-100">Open Attendance</a></div></div>
      <div class="col-md-4"><div class="pp-card pp-card-pad h-100"><h3 class="pp-section-title">🏖️ My Leave</h3><p class="text-muted small">{{pending}} pending request(s) · {{leave_balance}} day(s) available.</p><a href="/employee/leave" class="btn btn-sm pp-btn-primary w-100">Manage Leave</a></div></div>
      <div class="col-md-4"><div class="pp-card pp-card-pad h-100"><h3 class="pp-section-title">💰 Payslip</h3><p class="text-muted small">Open your latest salary breakdown.</p><a href="/employee/payslip_page" class="btn btn-sm pp-btn-primary w-100">View Payslip</a></div></div>
    </div>
    """, e=emp, s=stats, leave_balance=leave_balance_left, pending=pending_count,
       open_log=open_log, current_month=current_month, month_attendance=month_attendance, attendance_rows=attendance_rows)
