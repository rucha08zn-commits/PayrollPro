...
def ensure_current_month_demo_attendance():
    """Create lightweight demo attendance for the current month if it is empty.

    This fixes reused/deployed databases where employees already exist but the
    current-month attendance table has no rows, which made employee dashboards
    display 0 Present / 0 Absent even though demo attendance existed for August.
    Existing attendance and punch rows are never overwritten.
    """
    today = date.today()
    month_prefix = today.strftime("%Y-%m-")
    if Attendance.query.filter(Attendance.date.like(f"{month_prefix}%")).first():
        return
    ...
    for e in employees:
        ...
        while day <= today:
            ds = day.isoformat()
            if day.weekday() < 5 and ds not in holiday_dates:
                # Give most employees Present days, with a few deterministic
                # Absent/Leave rows so the dashboard visibly demonstrates all
                # attendance states.
                if e.id <= 12 and (e.id + day.day) % 17 == 0:
                    status = "Absent"
                elif e.id <= 12 and (e.id + day.day) % 23 == 0:
                    status = "Leave"
                else:
                    status = "Present"
                if not Attendance.query.filter_by(emp_id=e.id, date=ds).first():
                    db.session.add(Attendance(emp_id=e.id, date=ds, status=status, source="Demo-Current-Month"))
                if status == "Present" and not PunchLog.query.filter_by(emp_id=e.id, date=ds).first():
                    m = (e.id + day.day) % 5
                    db.session.add(PunchLog(emp_id=e.id, date=ds, time_in=f"09:{m:02d}:00", time_out=f"18:{m:02d}:00", hours=9.0))
